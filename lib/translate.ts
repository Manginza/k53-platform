...
function collectTextNodes(root: Node): Text[] {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(n: Node) {
      ...
      if (translated.has(n as Text)) return NodeFilter.FILTER_REJECT
...
/** Translate all untranslated visible text on the page into `lang`. */
export async function translatePage(lang: string): Promise<void> {
  ...
  try {
    ...
    // Apply every node from cache.
    for (const it of items) {
      const t = cache.get(key(lang, it.core))
      if (t == null) continue
      it.node.nodeValue = it.lead + t + it.trail

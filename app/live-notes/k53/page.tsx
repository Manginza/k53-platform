...
export default async function K53UnpackedPage() {
  ...
  const [{ data: chapters }, { data: quizzes }] = await Promise.all([
    supabase.from('ku_chapters').select('*').eq('is_front_matter', false).order('display_order'),
    supabase.from('ku_quizzes').select('id, chapter_id'),
  ])

  const progressData = user
    ? (await supabase.from('ku_user_chapter_progress').select('chapter_id, marked_complete, pages_read').eq('user_id', user.id)).data
    : []
  const attempts = user
    ? (await supabase.from('ku_quiz_attempts').select('quiz_id, score_percent, passed').eq('user_id', user.id).order('completed_at', { ascending: false })).data
    : []
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      ...
      <div className="mb-6 sm:mb-8">
        <Link href="/live-notes" className="text-xs text-gray-400 hover:text-green-600">‹ Live Notes</Link>
        <span className="block text-xs font-bold text-green-600 uppercase tracking-widest mt-2">K53 Unpacked</span>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mt-1 mb-2">
          Learner&apos;s &amp; K53 Driving Licence Manual
        </h1>
        <p className="text-sm sm:text-base text-gray-500">
          The complete K53 study guide — {totalChapters} chapters · exam-standard quiz after every chapter
        </p>
      </div>
              ...
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-gray-400 mt-0.5 hidden sm:block">
                    {ch.total_pages} page{ch.total_pages === 1 ? '' : 's'}

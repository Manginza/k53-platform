...
/**
 * AffiliateDashboard — referral link, headline stats and commission history.
 * Receives server-fetched data as props; only the copy-to-clipboard
...
export default function AffiliateDashboard({
  ...
  // Build the share link from the current origin so it works on any deploy
  const link = useMemo(() => {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          You earn {(affiliate.commission_rate * 100).toFixed(0)}% on every subscription bought through your link.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-md p-5">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button
            onClick={copy}
            className="bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg text-sm hover:bg-blue-800 transition-colors shrink-0"
          >
            {copied ? 'Copied!' : 'Copy link'}
          </button>
        </div>
        <p className="text-xs text-gray-400 mt-2">
          Code: <span className="font-mono font-semibold text-gray-600">{affiliate.code}</span>
        </p>
      </div>
